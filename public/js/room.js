...
const whiteboardButt = document.querySelector('.whiteboard-icon')
...
function handleVideoOffer(offer, sid, cname, micinf, vidinf) {
    ...
    connections[sid].ontrack = function (event) {
        ...
        if (!document.getElementById(sid)) {
            ...
            videoOff.innerHTML = '<i class="fas fa-video-slash"></i> Video Off';
...
videoButt.addEventListener('click', () => {
    ...
    if (videoAllowed) {
        ...
        videoAllowed = 1;
        videoButt.style.backgroundColor = "#800080";
        if (mystream) {
            mystream.getTracks().forEach(track => {
                if (track.kind === 'video')
                    track.enabled = true;
...
audioButt.addEventListener('click', () => {
    ...
    if (audioAllowed) {
        ...
        audioAllowed = 1;
        audioButt.style.backgroundColor = "#800080";
        if (mystream) {
            mystream.getTracks().forEach(track => {
                if (track.kind === 'audio')
                    track.enabled = true;
